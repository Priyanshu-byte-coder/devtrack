...
describe("GoalTracker - useGoalTracker Hook", () => {
  ...
  it("loads goals and handles auto-sync on mount", async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith("/api/goals/sync", {
      method: "POST",
    });
  });
  ...
  describe("getCompletionLabel calculation bounds", () => {
    it("returns completed and surpassed tags correctly", () => {
      ...
      expect(result.current.getCompletionLabel(weekly)).toBe(
        "Completed this week ✓"
      );
      expect(result.current.getCompletionLabel(monthly)).toBe(
        "Completed this month ✓"
      );
      expect(result.current.getCompletionLabel(surpassed)).toBe(
        "Goal surpassed! 🎯"
      );
    });
    ...
    it("returns deadline-based tags correctly", () => {
      ...
      expect(result.current.getCompletionLabel(tomorrowGoal)).toContain(
        "1d left"
      );
      expect(result.current.getCompletionLabel(todayGoal)).toBe("Due today ⏳");
      expect(result.current.getCompletionLabel(yesterdayGoal)).toBe(
        "Overdue ⚠️"
      );
    });
  });
